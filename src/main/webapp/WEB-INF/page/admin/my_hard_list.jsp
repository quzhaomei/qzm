...
//			whole:true,
			...
			normalOperator:[],
			 specialOperator:[{type:1,icon:"icon-insert_drive_file toorder",powerCode:"myWork-hard-order",title:"派单",
...
function toOrder(data){
	...
	$.post("myWork/hard-index.htmls?operator=one",{houseId:data.houseId},function(json){
		$.extend(data,json);
...
$(function(){
	...
	$("#searchStoreNameBtn").on("click",function(){//店铺搜索
		...
		$.post("myWork/hard-index.htmls?operator=company",param,function(json){
			//初始化店铺
	...
	$("#doOrderModalBtn").on("click",function(){
		...
		$.post("myWork/hard-order.htmls",param,function(json){
