...
var props={
			title:"软装需求列表",
			url:"requireTotal/soft-index.htmls?operator=list",
			head:[ 
			...
			normalOperator:[{icon:"icon-eye",title:"详细",powerCode:"requireTotal-soft-set-index",
			 ...
			 specialOperator:[{type:1,icon:"icon-insert_drive_file toorder",powerCode:"requireTotal-soft-set-order",
...
function closeRequire(data){
	...
	if(confirm("确定关闭该需求吗？")){
		$.post("requireTotal/soft-close.htmls",{houseId:data.houseId},function(json){
			alert(json.message);
			if(json.status==1){
...
function toEdit(data){
	$.post("requireTotal/soft-index.htmls?operator=one",{houseId:data.houseId},function(json){
		...
		$("#orderStatus_detail .label[status='"+data.softStatus+"']").show();
	},"json");
}
...
$(function(){
	...
	$("#searchStoreNameBtn").on("click",function(){//店铺搜索
		...
		$.post("requireTotal/soft-index.htmls?operator=company",param,function(json){
			//初始化店铺
	...
	$("#doOrderModalBtn").on("click",function(){
		...
		$.post("requireTotal/soft-order.htmls",param,function(json){
