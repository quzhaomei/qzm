...
var props={
			title:"软装需求列表",
			url:"myWork/soft-index.htmls?operator=list",
			head:[ 
			      ...
			      {name:"softStatus",title:"状态",style:"label",className:"td-label",search:true,
			    	  group:[{className:"label label-gray label-line",title:"待派单",value:"0"}
			    	  		,{className:"label label-gray",title:"已派单",value:"1"}
			    	  		,{className:"label label-red",title:"关闭",value:"2"}]}
			      ],
			normalOperator:[],
			 specialOperator:[{type:1,icon:"icon-insert_drive_file toorder",powerCode:"myWork-soft-order",
				 title:"派单",filter:{column:"softStatus",values:[0,1]},
						 callback:function(data){
							toOrder(data);
							 }
					 },{type:1,icon:"icon-lock_outline close-needs",title:"关闭",powerCode:"myWork-soft-close",
						 filter:{column:"softStatus",values:[0]},
							callback:function(data){
									closeRequire(data);
								}}
						]
...
function closeRequire(data){
	if(data.softStatus==2){alert("需求已经关闭，无法操作！");return;}
	
	if(confirm("确定关闭该需求吗？")){
		$.post("myWork/soft-close.htmls",{houseId:data.houseId},function(json){
			alert(json.message);
			if(json.status==1){
...
function toOrder(data){
	$.post("myWork/soft-index.htmls?operator=one",{houseId:data.houseId},function(json){
		$.extend(data,json);
		...
		$("#orderStatus .label[status='"+data.softStatus+"']").show();
		//初始化店铺
...
$(function(){
	...
	$("#searchStoreNameBtn").on("click",function(){//店铺搜索
		...
		$.post("myWork/soft-index.htmls?operator=company",param,function(json){
			//初始化店铺
	...
	$("#doOrderModalBtn").on("click",function(){
		...
		if(house.softStatus==2){alert("需求已经关闭，无法派单！");return;}
		...
		$.post("myWork/soft-order.htmls",param,function(json){
			...
			if(json.status==1){
				...
				$("#orderStatus .label[status=1]").show();
			...
			<div class="panel">
				<div class="panel-title">
					...
					<div class="label-right" id="orderStatus">
						...
						<div class="label" status="1" style="display:none;">
							已派单
						</div>
						
						<div class="label label-red" style="display:none;" status="2">
							关闭
						</div>
						<div class="label label-gray" style="display:none;" status="0">
							待派单
						</div>
					</div>
