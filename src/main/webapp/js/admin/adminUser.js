	...
	//把数组按下标的形式 放入json中
	function formatArr(json,arr,key){
		if(arr instanceof Array){
			for(var index=0;index<arr.length;index++){
				json[key+"["+index+"]"]=arr[index];
			}
		}
	}
	...
	$(".addSubmit").on("click",function(){//提交添加
			if($("#addForm").valid()){//验证通过！
				...
				formatArr(param, $("#add_roleList").val(), "roleIds");
				...
				$.post("add.htmls",param,function(json){//发送数据
					if(json.status==1){
	...
	$(".updateSubmit").on("click",function(){//提交修改
		...
		if($("#edit_roleArr").val()||bak.length!=0){//角色,
			if($("#edit_roleArr").val()&&bak){//比较俩数组
				if($("#edit_roleArr").val().length!=bak.length){
					roleChange=true;
					formatArr(param, $("#edit_roleArr").val(), "roleIds");
				}else{//长度一样，比较内容
		...
		$.post("update.htmls",param,function(json){//发送数据
			if(json.status==1){
				layer.msg(json.message,{ icon: 1,time: 1000 },function(){
					reset();
					});
	...
	$(".changeStatu").click(function(){
		...
		layer.confirm(tip, {
			...
			$.post("status.htmls",param,function(data){
			if(data.status==1){
	...
	$(".delete").click(function(){
		...
		layer.confirm(tip, {
			...
			$.post("delete.htmls",param,function(data){
				if(data.status==1){
					layer.msg(data.message,{ icon: 1,time: 1000 },function(){
						reset();
					});
	...
	$(".reset").click(function(){
		...
		layer.confirm("确定重置密码吗？", {
			...
			$.post("reset.htmls",param,function(data){
			if(data.status==1){
				layer.msg(data.message,{ icon: 1,time: 1000 },function(){
					reset();
				});
	...
	$(".editUser").click(function(){
		var userId=$(this).attr("userId");
	 	if(userId){
	 		...
	 		param.userId=userId;
			$.post("update.htmls",param,function(json){
				if(json.status==1){
					bak=json.data;
						//修改框 数据初始化
